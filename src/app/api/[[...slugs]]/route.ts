...
const rooms = new Elysia({ prefix: '/room' })
  ...
  .delete(
    ...
    async ({ auth }) => {
      ...
      await Promise.all([redis.del(roomId), redis.del(`meta:${roomId}`), redis.del(`messages:${roomId}`)])
    },
    { query: z.object({ roomId: z.string() }) }
  )
...
const messages = new Elysia({ prefix: '/messages' })
  ...
  .post(
    ...
    async ({ body, auth }) => {
      ...
      await Promise.all([redis.expire(`messages:${roomId}`, remainingTime), redis.expire(roomId, remainingTime)])
    },
    {
      query: z.object({ roomId: z.string() }),
      body: z.object({
